...
# End-to-end driver for TPCH Q5 experiments (predicate on/off × static/dynamic × sf0.1/sf1 × VO).
...
#
# Usage:
#   ./run_q5_matrix.sh              # run all combos
#   BATCH_SIZE=5000 ./run_q5_matrix.sh   # override batch size
...
SQL_DIR="tods26-experiments/queries/tpch_query_5/sql_files"
...
while IFS= read -r f; do
  ...
  base=${base%.txt}
  VO_LIST+=("$base")
done < <(find tods26-experiments/queries/tpch_query_5/variable_orders -maxdepth 1 -type f -name "tpch_q5_*.txt" | sort)
