...
VO_CONFIGS = [
    ("tpch_q3cnt_vo1_order_cust", lambda: build_chain(["order", "cust"])),
    ("tpch_q3cnt_vo2_cust_order", lambda: build_chain(["cust", "order"])),
]
...
def write_vo_files():
    ...
    for existing in VO_DIR.glob("tpch_q3cnt_*.txt"):
        existing.unlink()
...
def write_sql_files():
    ...
    for existing in SQL_DIR.glob("tpch_q3cnt_*.sql"):
        existing.unlink()
    vo_files = [p.name for p in VO_DIR.glob("tpch_q3cnt_*.txt")]
    for vo in vo_files:
