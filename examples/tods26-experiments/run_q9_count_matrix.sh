...
# End-to-end driver for TPCH Q9 count variant (no group-by; SELECT SUM(1)).
# Builds generated C++ and binaries, then runs with logging enabled.
#
# Outputs:
#   - Logs: tods26-experiments/output/q9count_<scale>_<mode>_<pred>.csv
#   - Binaries: bin/tods26count/
#   - Generated C++: generated/cpp/tods26count/
#
# Usage:
#   ./run_q9_count_matrix.sh              # run all combos
#   TEST_MODE=1 ./run_q9_count_matrix.sh  # run only first combo (smoke test)
...
SQL_DIR="tods26-experiments/queries/tpch_query_9_count/sql_files"
...
while IFS= read -r f; do
  ...
  base=${base%.txt}
  VO_LIST+=("$base")
done < <(find tods26-experiments/queries/tpch_query_9_count/variable_orders -maxdepth 1 -type f -name "tpch_q9cnt_*.txt" | sort)
...
build_and_run() {
  ...
  local log_out="${OUT_DIR}/q9count_${vo_name}_${scale}_${mode}_pred_${pred}.csv"

  echo "=== Q9COUNT ${vo_name} ${scale} ${mode} pred:${pred} ==="
  echo "SQL: ${sql_file}"

  ../scripts/generate-code.sh -l cpp -o "$cpp_out" "$sql_file"

  APP_INCLUDE=include/application/tpch/application_tpch_query9.hpp \
...
echo "Running Q9 COUNT variant with:"
