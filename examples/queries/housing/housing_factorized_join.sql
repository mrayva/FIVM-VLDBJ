...
FROM FILE './datasets/housing-4-normalised/House.tbl' LINE DELIMITED CSV(delimiter := '|');
...
FROM FILE './datasets/housing-4-normalised/Shop.tbl' LINE DELIMITED CSV(delimiter := '|');

CREATE STREAM INSTITUTION(postcode double, typeeducation double, sizeinstitution double)
FROM FILE './datasets/housing-4-normalised/Institution.tbl' LINE DELIMITED CSV(delimiter := '|');

CREATE STREAM RESTAURANT(postcode double, openinghoursrest double, pricerangerest double)
FROM FILE './datasets/housing-4-normalised/Restaurant.tbl' LINE DELIMITED CSV(delimiter := '|');
...
FROM FILE './datasets/housing-4-normalised/Demographics.tbl' LINE DELIMITED CSV(delimiter := '|');

CREATE STREAM TRANSPORT(postcode double, nbbuslines double, nbtrainstations double, distancecitycentre double)
FROM FILE './datasets/housing-4-normalised/Transport.tbl' LINE DELIMITED CSV(delimiter := '|');
