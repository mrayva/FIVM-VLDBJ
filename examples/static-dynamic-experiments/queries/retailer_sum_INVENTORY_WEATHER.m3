...
ON BATCH UPDATE OF WEATHER {
  TMP_SUM3(long)[][locn, dateid]<Local>  :=  AggSum([locn, dateid],
    (DELTA WEATHER)(locn, dateid, rain, snow, maxtemp, mintemp, meanwind, thunder)
  );
  V_locn_IIWLC1(long)[][]<Local>  +=  AggSum([],
    (AggSum([locn],
      (TMP_SUM3(long)[][locn, dateid]<Local> * V_ksn_II1(long)[][locn, dateid]<Local>)
    ) * V_zip_LC1(long)[][locn]<Local>)
  );
  V_rain_W1(long)[][locn, dateid]<Local>  +=  TMP_SUM3(long)[][locn, dateid]<Local>;
}
